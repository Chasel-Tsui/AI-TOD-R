...
from mmcv.ops import batched_nms, ModulatedDeformConvG
...
class OrientedRPN_DCFLHead(RotatedRPNHead):
    ...
    def _init_layers(self):
        ...
        self.rpn_conv = ModulatedDeformConvG(
            self.in_channels, self.feat_channels, 3, stride=1, padding=1, groups=1, bias=False)
        self.rpn_cls = nn.Conv2d(self.feat_channels,
                                 self.num_anchors * self.cls_out_channels, 1)
        self.rpn_reg = nn.Conv2d(self.feat_channels, self.num_anchors * 6, 1)
    def forward_single(self, x):
        """Forward feature map of a single scale level."""
        x, offsets, masks = self.rpn_conv(x)
        x = F.relu(x, inplace=True)
        rpn_cls_score = self.rpn_cls(x)
        rpn_bbox_pred = self.rpn_reg(x)
        return rpn_cls_score, rpn_bbox_pred, offsets
    ...
    def _get_targets_single(self,
        ...
        flat_anchors[...,2] = flat_anchors[...,2] + dx
        flat_anchors[...,1] = flat_anchors[...,1] + dy
        flat_anchors[...,3] = flat_anchors[...,3] + dy 

        deformable_anchors = flat_anchors[inside_flags,:]
        flat_bbox_preds = flat_bbox_preds[:,inside_flags]
        flat_cls_scores = flat_cls_scores[:,inside_flags]
    ...
    def get_targets(self,
        ...
        for i in range(len(cls_scores_list)):
            ...
            reshaped_bboxes = bbox_pred_list[i].detach().reshape(num_imgs,6,-1)
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
                    bbox_targets, bbox_weights, img_meta, num_total_samples):
